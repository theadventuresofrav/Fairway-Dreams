...
import { Friend, FriendData } from '../types';

interface AddEditFriendScreenProps {
  onSave: (data: FriendData) => void;
  initialData: Friend | null;
  onCancel: () => void;
}

const AddEditFriendScreen: React.FC<AddEditFriendScreenProps> = ({ onSave, initialData, onCancel }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Explicitly clear fields when there's no initial data,
      // for example when switching from an edit to a new add.
      setFullName('');
      setBirthDate('');
      setBirthTime('');
      setNotes('');
  ...
  return (
    <div className="bg-[#000000] rounded-2xl p-6 sm:p-8 shadow-2xl mb-6">
        <h2 className="mb-6 text-2xl font-bold text-center text-white">{isEditing ? 'Edit Friend' : 'Add a New Friend'}</h2>
        <p className="text-center text-gray-400 mb-6 -mt-4">{isEditing ? "Update your friend's details below." : "Add a friend's details to get support insights."}</p>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-5">
            ...
            <textarea
                id="notes"
                placeholder="e.g., Currently navigating a **major life event**. Prefers *quiet support* and check-ins."
                value={notes}
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-6">
            ...
            >
                {isEditing ? 'Save Changes' : 'Add Friend to Hub'}
            </button>
          </div>
        </form>
    </div>
...
export default AddEditFriendScreen;