...
interface FamilyMemberDetailViewProps {
    familyMember: FamilyMember;
    onBack: () => void;
}

const FamilyMemberDetailView: React.FC<FamilyMemberDetailViewProps> = ({ familyMember, onBack }) => {
    ...
    const fetchDetails = useCallback(async () => {
        ...
        setError(null);
        try {
            const initialAnalyses = analyzeCurrentChallenges(familyMember);
            ...
            console.error("Failed to fetch family member details:", err);
            ...
            <div className="space-y-6">
                {familyMember.notes && (
                    <div className="bg-[#1A1A1A] p-4 rounded-lg border border-gray-700">
                        ...
                        <p className="text-gray-300 italic" dangerouslySetInnerHTML={{ __html: formatNotes(familyMember.notes) }}></p>
                    </div>
                )}
                {analyses.map((analysis, index) => (
                    <div key={index} className="border-l-4 border-[#FF7A2F] pl-4">
                        ...
                        <div className="mt-3 bg-[#1A1A1A] p-4 rounded-lg">
                            <h4 className="font-semibold text-[#C6E0C2]">How to Be Supportive:</h4>
    ...
    return (
        <div className="bg-[#000000] rounded-2xl p-6 sm:p-8 shadow-2xl">
            <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-white">{familyMember.fullName}'s Cosmic Weather</h2>
            <p className="text-md text-gray-400 mb-6">Born on {new Date(familyMember.birthDate).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}</p>
...
export default FamilyMemberDetailView;